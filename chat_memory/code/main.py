"""
File: memory.py
Description: This module shows different memory management strategies as stuff everything, trimming, and summerize.
...
from paths import APP_CONFIG_FPATH, DATA_DIR, OUTPUTS_DIR, PROMPT_CONFIG_FPATH
from prompt_builder import build_system_prompt_from_config
from utils import load_env, load_publication, load_yaml_config, save_text_to_file


def count_tokens(text: str, model: str = "gemini-1.5-flash"):
...
def messages_to_string(messages: list, include_publication: bool = False) -> str:
    ...
    for i, msg in enumerate(messages):
        if isinstance(msg, SystemMessage):
            ...
            if (
                not include_publication
                and "=== PUBLICATION CONTENT ===" in system_content
            ):
...
def apply_trimming_strategy(
    conversation: list, system_prompt: str, window_size: int = 8
) -> list:
    """Keep only recent N messages. sliding window technique"""
...
def apply_summarization_strategy(
    conversation: list, system_prompt: str, llm, max_tokens: int = 1000
) -> list:
...
Focus on main topics and key information. Keep under 200 words."""
        ...
        print(f"  Summarization failed, using trimming: {e}")
...
def run_memory_strategy_conversation(
    ...
    if not system_prompt_config:
        raise ValueError(
            f"System prompt config '{system_prompt_config_name}' not found"
        )

    # Build system prompt
    system_prompt = build_system_prompt_from_config(
        system_prompt_config, publication_content
    )
    ...
    print(
        f"Running Strategy: {strategy_name.upper()} | Questions: {len(user_questions)}"
    )
    ...
    for idx, user_input in enumerate(user_questions, 1):
        print(
            f"  Processing question {idx}/{len(user_questions)}: {user_input[:50]}..."
        )
        ...
        if strategy_name == "stuffing":
            current_messages = apply_stuffing_strategy(
                conversation_history[:-1], system_prompt
            )
        elif strategy_name == "trimming":
            current_messages = apply_trimming_strategy(
                conversation_history[:-1], system_prompt, window_size
            )
        elif strategy_name == "summarization":
            current_messages = apply_summarization_strategy(
                conversation_history[:-1], system_prompt, llm, max_tokens
            )
        else:
        ...
        try:
            ...
            # Track this Q&A
            qa_pairs.append({"question": user_input, "response": response.content})
            ...
            if idx % 5 == 0:
                print(
                    f"   Completed {idx} questions, current prompt: {prompt_tokens:,} tokens"
                )
    ...
    if user_questions:
        if strategy_name == "stuffing":
            final_messages = apply_stuffing_strategy(
                conversation_history[:-1], system_prompt
            )
        elif strategy_name == "trimming":
            final_messages = apply_trimming_strategy(
                conversation_history[:-1], system_prompt, window_size
            )
        elif strategy_name == "summarization":
        ...
        final_prompt = messages_to_string(
            final_messages, include_publication=False
        )  # Exclude publication for readability
        final_response = (
            conversation_history[-1].content if conversation_history else "No response"
        )
    else:
        final_prompt = ""
        final_response = ""
...
def save_strategy_results(
    ...
    for token_data in token_progression:
        content.append(
            f"| {token_data['question_num']} | {token_data['prompt_tokens']:,} | {token_data['response_tokens']:,} | {token_data['total_tokens']:,} |"
        )
    content.append("")
    ...
    # Save file
    filename = f"{strategy_name}_results.md"
    save_text_to_file(
        "\n".join(content),
        os.path.join(OUTPUTS_DIR, filename),
        header=f"{strategy_name.title()} Strategy Results",
    )
...
def save_comparison_stats(all_stats: list):
    ...
    for stats in all_stats:
        content.append(
            f"| {stats['strategy'].title()} | {stats['total_prompt_tokens']:,} | {stats['total_response_tokens']:,} | {stats['total_tokens']:,} | {stats['questions_processed']} |"
        )
    ...
    if len(all_stats) > 1:
        baseline = next(
            (s for s in all_stats if s["strategy"] == "stuffing"), all_stats[0]
        )
        ...
        for stats in all_stats:
            if stats["strategy"] != baseline["strategy"]:
                savings = (
                    (baseline["total_tokens"] - stats["total_tokens"])
                    / baseline["total_tokens"]
                ) * 100
                content.append(
                    f"- **{stats['strategy'].title()}** vs {baseline['strategy'].title()}: {savings:.1f}% token savings"
                )
        content.append("")
    ...
    save_text_to_file(
        "\n".join(content),
        os.path.join(OUTPUTS_DIR, "memory_comparison_stats.md"),
        header="Memory Strategy Comparison Statistics",
    )
    print("Comparison statistics saved to memory_comparison_stats.md")
...
def run_single_strategy():
    ...
    model_name = app_config.get("llm", "gemini-1.5-flash")
    ...
    # Load questions
    questions_config = load_yaml_config(
        os.path.join(DATA_DIR, "yzN0OCQT7hUS-sample-questions.yaml")
    )
    user_questions = questions_config.get("questions", [])

    # Ask how many questions
    num_questions = input(
        f"How many questions to process? (max {len(user_questions)}, default=10): "
    ).strip()
    try:
...
def run_comparison():
    ...
    for stats in all_stats:
        print(
            f"{stats['strategy'].title():15} | {stats['total_tokens']:,} total tokens"
        )
...
def main():
    """Main entry point."""
    print("=" * 80)
    print("MEMORY STRATEGY DEMONSTRATION")
    print("=" * 80)
